...
export default function ActionLink({ children, href, className, iconClassName, icon: Icon = ArrowUpRight, ...props }) {
  ...
  return (
    <Component
      href={href}
      className={cn(
        "group/btn flex w-max items-center gap-4 rounded-xl border border-white/10 bg-white/10 py-1.5 pl-6 pr-1.5 shadow-[0_4px_16px_rgba(0,0,0,0.3)] backdrop-blur-md transition-all duration-300 hover:border-white/20 hover:bg-black/30",
        className
      )}
      {...props}
    >
